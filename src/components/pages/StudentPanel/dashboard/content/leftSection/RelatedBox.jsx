...
const RelatedBox = ({ bg, title, _id }) => {
  return (
    <Link
      href={`/courses/${_id}`}
      className={`${bg} cursor-pointer hover:bg-color-green transition-all duration-200  z-2 relative mt-2 rounded-lg hlg:py-4 ss:py-2 py-4 hlg:px-4 px-2 shadow-lg flex items-center`}>
      ...
      <div className="flex-1 text-white pl-3">
        <h3 className=" font-bold text-[15px] hlg:text-[18px]">{title}</h3>
      </div>
