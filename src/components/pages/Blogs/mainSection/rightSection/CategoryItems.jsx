  ...
  return (
    <>
      {categoryItemsInfo.map((item, index) => (
        <Link
          href={`/blogs?limit=${limit}&page=${page}&category=${item.link}`}
          key={item.id}
          className={`${
            index < categoryItemsInfo.length - 1 && "mb-4"
          } border-b-[1px] hover:pl-4 ${
            item.link == category &&
            " text-primary pl-4 font-bold border-primary border-b-[2px]"
          } transition-all hover:text-primary hover:border-primary hover:font-bold cursor-pointer duration-500 border-[#77777789] pb-4 text-[14px] text-[#8e8e8e] flex justify-between items-center`}>
          <li>{item.title}</li>
          <li>({item.count})</li>
        </Link>
      ))}
